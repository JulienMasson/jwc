...
# install xcb-proto version 1.13.1
git clone https://anongit.freedesktop.org/git/xcb/proto.git
pushd proto
git checkout xcb-proto-1.13
autotools_install
popd

# install libxcb version 1.13.1
git clone https://anongit.freedesktop.org/git/xcb/libxcb.git
pushd libxcb
git checkout 1.13.1
autotools_install --without-doxygen
popd
